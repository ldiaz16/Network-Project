...
flowchart LR
    ...
    DataStore -->|"T_T100_SEGMENT_ALL_CARRIER.csv(.gz)"| DataDir["Repo root / BTS dataset"]
```
...
- **Data access**: `src/load_data.py` ingest the BTS T-100 segment export (`T_T100_SEGMENT_ALL_CARRIER.csv` or `T_T100_SEGMENT_ALL_CARRIER.csv.gz`) and related lookup tables (`Lookup Tables/L_UNIQUE_CARRIERS.csv`, `Lookup Tables/L_AIRPORT.csv`) to enrich the routes with airport metadata.
